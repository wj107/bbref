#######################################################################################################
#### script to clean data frame of play-by-play data from a gamelink in basketball-reference.com
#######################################################################################################
...
#---get html table w/play-by-play data from gamelink, convert to data frame
  ...
  #--convert to data frame with data!!!
  dat<-as.data.frame(play.by.play.html)
...
#---figure out abbreviations...
#---home/away placeholders while abbreviations are assigned
	names(dat)[c(2,6)]<-c("at","ht")	
...
#---convert seconds from opening tip
	...
	time%*%MM->Sec
#---add seconds to the data frame!
	
	dat<-cbind(dat,Sec,Play,Team)

#---rearrange columns
	dat<-dat[,c(1,4,7,8,9,5,6)]
